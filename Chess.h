...
namespace ast {
	...
	struct visitor {
		...
		void operator()(castles_kingside const& v) {
			this->moves.emplace_back("castles_kingside");
		}
		void operator()(castles_queenside const& v) {
			this->moves.emplace_back("castles_queenside");
		}
		void operator()(pawn_move const& v) {
			this->moves.emplace_back("pawn_move " + std::string(v.begin(), v.end()));
		}
		void operator()(pawn_capture const& v) {
			this->moves.emplace_back("pawn_capture " + std::string(v.begin(), v.end()));
		}
		void operator()(pawn_capture_ep const& v) {
			this->moves.emplace_back("pawn_capture_en_passant " + std::string(v.begin(), v.end()));
		}
		void operator()(pawn_promotion const& v) {
			this->moves.emplace_back("pawn_promotion " + std::string(v.begin(), v.end()));
		}
		void operator()(pawn_capture_promotion const& v) {
			this->moves.emplace_back("pawn_capture_promotion " + std::string(v.begin(), v.end()));
		}
		void operator()(piece_move const& v) {
			this->moves.emplace_back("piece_move " + std::string(v.begin(), v.end()));
		}
		void operator()(piece_capture const& v) {
			this->moves.emplace_back("piece_capture " + std::string(v.begin(), v.end()));
		}
		void operator()(explicit_piece_move const& v) {
			this->moves.emplace_back("explicit_piece_move " + std::string(v.begin(), v.end()));
		}
